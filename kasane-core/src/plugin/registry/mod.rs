...
/// stack usage trivial. ADR-031 Phase 10 Step 2 — see "再帰深さ +
/// 循環検出" in the closing-notes section.
const MAX_INLINE_BOX_DEPTH: usize = 8;

thread_local! {
    /// Per-thread stack tracking the chain of `box_id`s currently being
    /// painted. Used by [`PluginView::paint_inline_box`] to detect
    /// self-cycles, mutual cycles between plugins, and runaway nesting.
    /// `Vec` (not `HashSet`) because the depth bound is small and
    /// linear scan beats hashing at this scale; ordering also lets us
    /// emit useful "owner=X box_id=Y" diagnostics.
    static INLINE_BOX_STACK: RefCell<InlineBoxStackInner> =
        RefCell::new(InlineBoxStackInner::default());
}

#[derive(Default)]
struct InlineBoxStackInner {
    ...
    logged_overflow: std::collections::HashSet<(String, u64)>,
    logged_cycle: std::collections::HashSet<(String, u64)>,
}

struct InlineBoxStack;

impl InlineBoxStack {
    fn with<R>(f: impl FnOnce(&mut InlineBoxStackInner) -> R) -> R {
...
impl InlineBoxStackInner {
    ...
    fn contains(&self, box_id: u64) -> bool {
        self.chain.contains(&box_id)
    }

    fn push(&mut self, box_id: u64) {
    ...
    fn log_overflow_once(&mut self, owner: &PluginId, box_id: u64) {
    ...
    fn log_cycle_once(&mut self, owner: &PluginId, box_id: u64) {
        let key = (owner.0.clone(), box_id);
...
#[cfg(test)]
mod inline_box_recursion_tests {
    //! Verify the recursion-depth and cycle guards on
    //! [`PluginView::paint_inline_box`]. These tests directly exercise
    //! [`InlineBoxStackInner`] (the thread-local guard state) since
    //! constructing a full `PluginView` with mock plugins for the same
    //! purpose adds significant fixture overhead without testing
