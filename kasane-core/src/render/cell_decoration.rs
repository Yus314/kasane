    ...
    for dec in decorations {
        match &dec.target {
            DecorationTarget::Cell(coord) => {
                ...
                {
                    cell.with_style_mut(|s| dec.merge.apply_to_terminal(s, &dec.style));
                }
            }
...
mod tests {
    ...
    fn cell_decoration_single_cell() {
        ...
        assert_eq!(cell.style.bg, Color::Rgb { r: 255, g: 0, b: 0 });
        // Other cells unchanged
        let other = grid.get(0, 0).unwrap();
        assert_eq!(other.style.bg, Color::Default);
    }
    ...
    fn replace_merge_mode() {
        ...
        if let Some(cell) = grid.get_mut(0, 0) {
            cell.style.fg = Color::Rgb { r: 0, g: 255, b: 0 };
        }
        let dec = make_decoration(
            DecorationTarget::Cell(Coord { line: 0, column: 0 }),
            FaceMerge::Replace,
