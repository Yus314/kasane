...
/// A declarative widget registered as a Plugin.
...
/// substring matches and applying a face override to each.
fn build_inline_decoration(
    ...
    // Concatenate atom contents to get line text, tracking byte offsets per atom.
    ...
    if pattern.is_empty() || full_text.is_empty() {
        return None;
    }

    // Find all non-overlapping matches and create Style ops.
    let mut ops = Vec::new();
    let mut search_start = 0;
    while let Some(pos) = full_text[search_start..].find(pattern) {
...
///
/// Each widget becomes a `WidgetPlugin` registered via `PluginBridge`.
/// Returns the set of widget names (for use in subsequent hot-reloads).
pub fn register_all_widgets(
    ...
    for (i, def) in file.widgets.into_iter().enumerate() {
        let bridge = if i == 0 && !diagnostics.is_empty() {
            PluginBridge::new(WidgetPlugin::new(def)).with_diagnostics(diagnostics.clone())
        } else {
            PluginBridge::new(WidgetPlugin::new(def))
        };
...
pub fn hot_reload_widgets(
    ...
    for old_name in old_names {
        if !new_names.iter().any(|n| n == old_name) {
            let id = WidgetPlugin::plugin_id_for(old_name);
