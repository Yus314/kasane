...
| `kasane.kdl` auto-reload for plugins/settings | Current | **Opt-in `plugins.auto_reload`** | Live `resolve` + plugin reload triggered by kdl edits when enabled; default is the prior "resolve + restart" workflow. Details in [ADR-040](#adr-040-kasanekdl-auto-reload-for-plugins-and-settings). |
...
removal is batched per [ADR-046](#adr-046-wit-abi-600-batched-retirement).

### Context

The plugin-defined extension-point API
([ADR-029](#adr-029-topic-based-pubsub-and-plugin-defined-extension-points))
...
Pub/sub
([`r.subscribe` / `r.publish_typed`](#adr-029-topic-based-pubsub-and-plugin-defined-extension-points))
...
[ADR-048](#adr-048-plugin-backend-trait-extinction-phase-β) committed
