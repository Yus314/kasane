...
## ADR-004A: 標準フロントエンド互換性を上位制約とする

**状態:** 決定済み

**コンテキスト:**
Kasane は一方で `kak = kasane` として既存の Kakoune 利用者がそのまま採用できることを理想とし、他方で plugin 作者に強い UI 拡張能力を提供したい。この 2 つを同一レイヤーで満たそうとすると、標準利用者の互換性か plugin platform の自由のどちらかが侵食される。

**決定:** Kasane は product としては標準フロントエンド互換性を第一義とし、plugin platform 性はその上に積む。すなわち、Default Frontend Semantics を上位制約とし、Extended Frontend Semantics は追加能力として位置づける。

**具体原則:**
- `kak = kasane` は bitwise 同一 UI ではなく semantic compatibility を意味する
- デフォルト状態では既存の `kakrc`、autoload、既存プラグイン、既存ワークフローとの整合性を優先する
- Kasane 独自の plugin、surface、再構成 UI は追加価値であり、通常利用の前提条件としない
- plugin-defined UI は protocol truth を改竄せず、display policy として core semantics に参加する
- 強い再構成や Observed-eliding transformation は opt-in の拡張意味論に属する

**根拠:**
- Kakoune コミュニティで広く採用されるためには、高度機能より導入摩擦の低さが重要
- 既存ユーザーにとっての価値は、新しい ecosystem への参加より、既存運用を壊さず UI を改善することにある
- plugin platform を product の第一義にすると、bundled plugin や独自 ecosystem が標準 frontend 意味論を侵食しやすい
- Default/Extended の二層を明示することで、保守的デフォルトと強い拡張性を同時に維持できる
...
- P-3: ✓ 完了 — ファジーファインダー参照実装 (`kasane-wasm/guests/fuzzy-finder/`)、`on_io_event` criterion ベンチマーク (`kasane-wasm/benches/io_event.rs`)、`perf_span!("deliver_io_event")` 計測ポイント、TUI/GUI フレームスパン + バッチドレインログ、`state_hash` キャッシュ不整合バグ修正 (adapter.rs: `on_io_event` / `handle_key` 後にキャッシュ更新)
