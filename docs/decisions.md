...
| 四層レイヤー責務 | **上流/コア/組み込み/外部プラグイン** | 機能の責務境界を四層モデルで判断。[layer-responsibilities.md](./layer-responsibilities.md) |
| WASM プラグインランタイム | **Component Model (wasmtime)** | ベンチマーク実証済み。10 プラグイン 18 μs (フレーム予算の 45%)。詳細は [ADR-013](#adr-013-wasm-プラグインランタイム--component-model-採用) |
...
**コンテキスト:**
Phase 4a/4b の項目分類で、機能がどの層に属するかの体系的な基準が必要になった。既存の「解決層」(requirements.md §2) は実装メカニズム (HOW) の分類であり、責務境界 (WHERE) の判断基準としては不十分。
...
| 組み込みプラグイン | API 実証 + 参照実装 + デフォルト UX | 未実証 API の検証を兼ねるか？ |
| 外部プラグイン | ユーザー固有・外部依存・実験的機能 | 実証済み API で実装可能か？ |
...
組み込みプラグインは `pub(crate)` 禁止。公開 Plugin trait API のみを使用する。API が不足している場合は API 拡張が先であり、バックドアは不可。

**根拠:**
- 外部プラグインが同等の機能を実装できることを保証する
- 組み込みプラグインが参照実装として機能するために必要
- プラグインエコシステムの信頼性を維持する
...
40 μs @ 80x24 のフレーム予算に対する WASM プラグインの占有率:

| プラグイン数 | WASM コスト | 予算占有率 | 残りの予算 |
|------------|-----------|----------|----------|
| 1 | 1.80 μs | 4.5% | 38.2 μs |
| 3 | 5.45 μs | 13.6% | 34.6 μs |
| 5 | 8.91 μs | 22.3% | 31.1 μs |
| 10 | 18.0 μs | 45.0% | 22.0 μs |
...
1. **絶対性能が十分**: 5 プラグインで予算の 22%、10 プラグインでも 45%。ホスト側パイプラインに十分な余裕がある。
...
- Component Model は軽量関数で 13-21x のオーバーヘッドを加える。ただし絶対値は ~550 ns であり、フレーム予算 (40 μs) の 1.4% にすぎない。
...
- ネイティブプラグインは最適化エスケープハッチとして維持 (パフォーマンスクリティカルな組み込みプラグイン向け)
