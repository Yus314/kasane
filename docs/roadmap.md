...
| β-3.3b.11 — WasmPlugin static metadata + cleanup | pending | Final per-family migration (id / view_deps / capabilities / authorities / capability_descriptor / allows_process_spawn / set_plugin_tag / drain_diagnostics) populating the same `register()`. See [`memory/project_phase_beta_3_3b_substaging.md`](../) for the breakdown. |
