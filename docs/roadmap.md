...
| Composable Lenses | **MVP + caching + 3 bundled lenses landed (2026-05-04)** — `kasane_core::lens` with `Lens` trait, `LensId`, `LensRegistry`, opt-in `CacheStrategy::PerBuffer` (per-buffer-content cache; buffer hash shared across `PerBuffer` lenses; cache entries dropped on disable / unregister / re-register). Bundled lenses: `TrailingWhitespaceLens`, `LongLineLens`, `IndentGuidesLens` (all opt in to `PerBuffer`). Native-only. Follow-ups: (a) finer `CacheStrategy::PerLine` granularity (needs per-line `Lens` method), (b) WIT surface for WASM plugins to register lenses, (c) more bundled example lenses (mixed-indent warning, tab marker, etc.). |
