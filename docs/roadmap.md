...
| β-3.3b.12 — WasmPluginLoader return-type flip | pending | Switch `WasmPluginLoader::load` to return `PluginBridge::new(WasmPlugin {…})` and delete `impl PluginBackend for WasmPlugin`. Update call sites in `kasane/src/locked_wasm_provider.rs`, `kasane-wasm/src/lib.rs::tests`, and `kasane-wasm/benches/io_event.rs`. After this lands, only `PluginBridge`'s `impl PluginBackend` remains. |
