...
| Phase P | プラグイン I/O 基盤 | P-1/P-2 完了 | P-3 (実証・安定化) が残。Phase 5 とは独立に進行可能 |
...
|---|---|---|---|---|
| P-1 | WASI ケイパビリティ基盤 | WASM ランタイム | ✓ 完了 | WIT v0.5.0 に `capability` enum + `requested-capabilities` export 追加。`WasiCapabilityConfig` + `build_wasi_ctx()` でプラグイン別の `preopened_dir` / `inherit_env` を設定。`config.toml` の `deny_capabilities` で拒否可能 |
| P-2 | プロセス実行基盤 | コア基盤 + プラグイン API | ✓ 完了 | `IoEvent` / `ProcessEvent` 型、`Plugin::on_io_event()`、`Command::SpawnProcess` 等 4 コマンド、tokio ベース `ProcessManager` (kasane/ crate)、TUI/GUI イベントループへの `ProcessOutput` 統合、`ProcessEventSink` / `ProcessDispatcher` trait 抽象化、WIT v0.6.0 に `io-event` + `on-io-event` 追加、`Capability::Process` のケイパビリティチェック (`allows_process_spawn`)、プラグイン別プロセス上限 (4/plugin, 16 total) |
