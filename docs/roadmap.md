...
| ADR-031 post-closure perf opportunities | (1) `StyledLine` allocation reuse (`RendererTextScratch` thread-local) — estimated 2–5 µs/frame on warm path. (2) `atom_styles: Vec<Arc<Style>>` (B-wide pattern extension to post-resolve). (3) Sub-line word/cluster shape cache — only path that structurally reduces `shape_warm = 13.58 µs` per L1 miss. SLO has 3.5× headroom (current 56.7 µs vs 200 µs); not on critical path |
| ADR-031 post-closure visibility tightening | `Face` → `WireFace` rename + `pub(in crate::protocol)` downgrade. Blocked on host-crate Face usage: kasane-wasm convert layer (~30 sites in `convert/mod.rs` + `host.rs` Host trait impl), kasane-tui / kasane-gui benches and diagnostics (~15 sites), kasane builtins. Migrate those sites to Style end-to-end first, then the rename + downgrade is mechanical. `#[doc(hidden)]` already keeps `Face` invisible from rendered API |
