...
| B3 commits 6-7 — bridge cleanup + visibility downgrade | In progress (`phase-b3-block-e`) | Internal-only cleanup: retire `Atom::from_face`/`face`, `Style::from_face`/`to_face`, `UnresolvedStyle::from_face`/`to_face`, `Theme::set/get/resolve` Face versions, `FaceMerge::apply` Face version, `Cell::face()` accessor, `From<Face> for Style`/`for ElementStyle`, `TerminalStyle::from_face`. Then downgrade `Face`/`Color`/`Attributes` from `pub` to `pub(in crate::protocol)`. ~250 test/bench refs cascade. `Atom::with_style(text, Style)` constructor landed (`c7e21b36`) as the migration vehicle |
