...
| Composable Lenses | **MVP + PerBuffer + PerLine caching + 3 bundled lenses landed (2026-05-04)** — `kasane_core::lens` with `Lens` trait, `LensId`, `LensRegistry`. Two opt-in cache strategies: `PerBuffer` (single entry per lens; whole-buffer hash) and `PerLine` (one entry per `(lens, line_idx)`; single-line edit invalidates one entry per lens). `PerLine` needs `Lens::display_line(view, line)` override for actual savings; default impl filters whole-buffer output by anchor line. Bundled lenses (`TrailingWhitespaceLens`, `LongLineLens`, `IndentGuidesLens`) all opt in to `PerLine` with optimised line-only overrides. Native-only. Follow-ups: (a) WIT surface for WASM plugins to register lenses, (b) more bundled example lenses (mixed-indent warning, tab marker, etc.). |
