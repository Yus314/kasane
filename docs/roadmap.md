...
- ADR-045: Retire `extension-point` API — **partially landed**. The Rust dispatch path (`PluginRuntime::evaluate_extensions`, the `PluginBackend::{extension_definitions,evaluate_extension}` hooks, the `HandlerRegistry::{define_extension,on_extension}` builder methods, and the `ExtensionDefinition` / `ExtensionContribution` / `ExtensionResults` / `ExtensionOutput` / `CompositionRule` types) has been deleted. `ExtensionPointId` survives because the plugin manifest schema still parses extension-point metadata. The WIT `evaluate-extension` guest export still ships in `kasane:plugin@5.0.0` and is scheduled for removal at the next major ABI bump (`6.0.0`), where it can be batched with other breaking surface changes per `docs/abi-versioning.md` ("Remove or rename a function" is a major bump). No `.wasm` rebuild is required for the partial removal — guests retain the unreferenced export.
