...
| Plugin ABI 4.0.0 — partially landed (2026-05-11) | From the sprout dogfooding tracker (Issue #81). [ADR-041](./decisions.md#adr-041-eval-command-in-session-ready-command) **Decided + landed (`dd2fbe3a`)**: `eval-command(string)` added to `session-ready-command`; ABI bumped 3.0.0 → 4.0.0; all 23 manifests + 6 bundled WASM plugins rebuilt. [ADR-042](./decisions.md#adr-042-command-error-event-via-info_show-marker-attribution) Phase A landed (host-internal marker recognition + suppression in `kasane-core/src/plugin/error_attribution.rs`); Phase B (host auto-wrap + `on-command-error-effects` WIT export) still **Proposed** — pending review and a 4.1.0 minor bump. |
