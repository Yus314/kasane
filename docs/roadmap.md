...
| β-3.3 — Delete `PluginBackend` trait + collapse bridge.rs | pending | Blocked on (a) defining a non-trait `PluginEntry` shape that `PluginRuntime` can store, (b) `WasmPlugin::into_entry()` rewrite producing that shape, and (c) `WidgetBackend` rewrite to the `WidgetPlugin` route. Once (a) lands, the `SlotImpl` enum collapses to `Vec<PluginEntry>` and the trait deletion is structural. Target: `bridge.rs` 964→200, `traits.rs` 846→0. |
