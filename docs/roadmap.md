...
| Plugin ABI 4.0.0 (RFCs proposed 2026-05-11) | Two ADRs in `Proposed` state, both from the sprout dogfooding tracker (Issue #81). [ADR-041](./decisions.md#adr-041-eval-command-in-session-ready-command) adds `eval-command(string)` to `session-ready-command` (1 WIT line + 1 host match arm). [ADR-042](./decisions.md#adr-042-command-error-event-via-info_show-marker-attribution) adds `command-error` record + `on-command-error-effects` export, host auto-wraps plugin-originated commands via the `info_show` marker-attribution protocol (Phase A of ADR-042 already landed: host-internal marker recognition + suppression in `kasane-core/src/plugin/error_attribution.rs`). Coordinated for a single ABI 3.0 → 4.0 plugin rebuild. Awaiting review. |
