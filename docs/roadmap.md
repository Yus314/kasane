...
> **スコープ方針:** 上流 (Kakoune) にブロックされている項目はロードマップから分離し、[upstream-dependencies.md](./upstream-dependencies.md) で追跡する。また、組み込みプラグインは「未実証の Plugin API extension point を検証する」目的で作成し、既に実証済みの API 上で作れる機能は Phase 5 以降で外部プラグインとして実現する。
...
| 項目 | 内容 | 実装方針 |
|------|------|----------|
| R-027 | 起動時 info キューイング | TEA update() にキューを導入。UI 準備完了前に受信した `info_show` を保持し、初回 `refresh` 後に表示 |
| R-050 | 複数カーソル描画 (完全実装) | `draw` メッセージの PrimaryCursor/SecondaryCursor face を解析し、paint() でカーソルスタイル (ブロック/バー/アンダーライン) を正確にレンダリング。セカンダリカーソルは薄い背景色で区別 |
| R-051 | フォーカス連動カーソル | ウィンドウフォーカス喪失を検知し (crossterm `FocusLost` / winit `Focused(false)`)、カーソルをアウトラインスタイルに切り替え |
| E-040 | アンダーラインバリエーション | Face の `underline` 属性に波線 (curly)・点線 (dotted)・二重線 (double) を追加。TUI は Unicode 近似文字、GUI はシェーダーで描画 |

**組み込みプラグイン (未実証 API の検証):**

| 項目 | 実証する API | 内容 |
|------|------------|------|
| R-052 | `Slot::BufferTop` / `BufferBottom` | ビューポート外カーソルの方向と数をバッファ端に表示。カーソル位置とビューポート範囲の比較が必要 (内部状態アクセス) |
| E-006 | `contribute_line()` 拡張 (選択範囲) | 改行を含む選択範囲をウィンドウ幅いっぱいまでハイライト。CursorLinePlugin の拡張として実装可能 |
| E-005 | `OverlayAnchor::Absolute` | ビューポート座標に対するオーバーレイ描画。easymotion ジャンプラベル等のユースケース。API 設計は済んでおり実装が必要 |
