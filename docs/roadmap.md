...
> **現状:** コンパイル時結合方式の外部プラグインは実現済み。`kasane-core::plugin_prelude` + `#[kasane_plugin]` マクロで外部クレートからプラグインを定義し、`kasane::run(|registry| { ... })` でカスタムバイナリとして配布できる。詳細は [plugin-development.md](./plugin-development.md) および `examples/line-numbers/` を参照。
>
...
**Phase W1 — ✓ 完了:**
- WIT インターフェース v0.2.0 (kasane の Plugin trait 相当を WIT で定義)
...
> **Note:** ネイティブプラグイン (コンパイル時結合方式) は Decorator/Replacement 等 WIT 未公開 API のためのエスケープハッチとして維持する。
