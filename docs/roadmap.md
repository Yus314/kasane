...
- ADR-045: Retire `extension-point` API (dispatch wiring is live in `registry/mod.rs:evaluate_extensions` + bridge + WASM adapter + tests, but in-tree producer count is 0; deletion requires WIT 5.1.0 minor bump + 13 `.wasm` rebuild).
