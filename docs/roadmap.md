...
| β-3.3 — Delete `PluginBackend` trait + collapse bridge.rs | pending | Blocked on β-4 (WasmPlugin still implements PluginBackend) + WidgetBackend rewrite to WidgetPlugin route. Target: `bridge.rs` 964→200 prod, `traits.rs` 846→150. |
