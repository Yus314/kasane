...
| β-3.3 — Delete `PluginBackend` trait + collapse bridge.rs | design landed (`1b84b833`), implementation pending | [ADR-049](decisions.md#adr-049-pluginentry-shape-and-β-33-staging) settles the question: no separate `PluginEntry` type — `PluginBridge` itself becomes the storage shape by extracting `impl PluginBackend` into inherent methods. Sub-phases β-3.3a-e enumerated. Remaining work: (a) WidgetBackend deletion + 22 widget/tests.rs rewrites, (b) WasmPlugin migration to produce `PluginBridge` from `WasmPluginLoader::load`, (c-e) bridge.rs collapse + trait deletion. Target: `bridge.rs` 964→200, `traits.rs` 846→0. |
