...
| B3 commits 6-7 — bridge cleanup + visibility downgrade | Pending | Retire `Style::from_face`/`to_face`/`to_face_with_attrs`, `UnresolvedStyle::to_face`, `Atom::face`, `Cell::face()`, `From<Face> for Style`/`for ElementStyle`, `TerminalStyle::from_face`, `kasane-tui::sgr::emit_sgr_diff(Face)` shim. `UnresolvedStyle::from_face` and `Atom::from_face` (wire-aware) stay — used by the parser and `final_*`-preserving fixtures. ~30 production callsites (inline `Face { ... }` literals in `render/cursor`, `inline_decoration`, `markup`, `walk_scene`, `ornament`, `halfblock`, plugin / widget defaults) + ~150 test refs cascade. Best executed compile-error-driven for uniform migration shape. After deletion, downgrade `Face`/`Color`/`Attributes` from `pub` to `pub(in crate::protocol)` |
