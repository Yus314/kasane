...
| β-3.3b.9-11 — WasmPlugin handler-family migrations | pending | Remaining per-family migrations (persistence + workspace, process tasks + pubsub + lens, static metadata + cleanup) populating the same `register()`. See [`memory/project_phase_beta_3_3b_substaging.md`](../) for the breakdown. |
