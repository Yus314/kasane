...
| β-3.3b-e — WasmPlugin migration + trait deletion + bridge collapse | pending | Long pole: `WasmPlugin` migration to produce `PluginBridge` from `WasmPluginLoader::load` (~75 WIT-export wirings). Then collapse `SlotImpl` to a single `PluginBridge` field, convert `impl PluginBackend for PluginBridge` to inherent methods, delete the trait, collapse `bridge.rs` 964→200. Target: ~-2000 LoC total. |
