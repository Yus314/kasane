...
**Level 3 enforcement (ADR-030, shipped).** `Command::is_kakoune_writing()` provides an exhaustive classification of every variant on the writing/transparent axis (no `_` wildcard — new variants force explicit classification). `TransparentCommand` is a newtype wrapping `Command` that restricts construction to the 26 non-writing variants; handlers returning `Vec<TransparentCommand>` provide a compile-time witness that they cannot emit Kakoune-writing commands. Five `_transparent` registration methods on `HandlerRegistry` (`on_key_transparent`, etc.) set `TransparencyFlags`, enabling `is_input_transparent()` to auto-derive T10 per plugin. Structural tests pin the classification constants, and an A3 property test (`kasane-core/tests/a3_transparent_tau.rs`) witnesses that non-deferred transparent commands produce zero Kakoune output. Note: `InjectInput` and `Session(Switch)` are classified as transparent (they do not write to Kakoune directly); transitive writing analysis is deferred to Level 5 (free monad).

### 13.14 Visual Faithfulness Recovery Witness

The Visual Faithfulness condition (§10.2a) requires every active Destructive Display Transformation to be recoverable by a bounded user interaction sequence.

**Level 4 enforcement (ADR-030, shipped).** `DisplayDirective::is_destructive()` provides an exhaustive classification of every variant on the destructive/non-destructive axis (no `_` wildcard — new variants force explicit classification). `SafeDisplayDirective` is a newtype wrapping `DisplayDirective` that restricts construction to the three non-destructive variants (`Fold`, `InsertAfter`, `InsertBefore`); there is no `Hide` constructor. `RecoveryWitness` paired with `RecoveryMechanism` provides registration-time evidence that destructive directives are recoverable. Three registration methods on `HandlerRegistry` (`on_display`, `on_display_safe`, `on_display_witnessed`) set `RecoveryFlags` on the handler table; `is_display_recoverable()` auto-derives Visual Faithfulness per plugin. `Fold` is classified as Preserving (not Destructive) because `FoldToggleState` provides framework-maintained recovery: a single toggle interaction restores all folded lines. Structural tests pin the classification constants, and a property test (`kasane-core/tests/visual_faithfulness.rs`) witnesses that fold toggle recovers all folded lines in a single interaction.

### 13.15 Plugin Transparency Is Partially Statically Decidable

T10 (Plugin Transparency, §12.10) classifies a plugin as Kakoune-transparent iff none of its handlers emit Kakoune-writing Commands. For plugins using `HandlerRegistry`, the `_transparent` registration methods (ADR-030 Level 3) set `TransparencyFlags` on the handler table; `HandlerRegistry::is_input_transparent()` returns true when all registered input handlers used their transparent variants. View handlers (contribute, transform, annotate, overlay, display) are transparent by construction since they never return Commands. Lifecycle handlers (`on_state_changed` etc.) are not yet covered — they have legitimate Kakoune-writing use cases. Full static analysis via free monad (Level 5) would close the remaining gap.
