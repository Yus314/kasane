...
`kasane:plugin@6.3.0` (adds `emit-diagnostic` plugin-emitted diagnostic
command per [#106](https://github.com/Yus314/kasane/issues/106)).
ABI 6.2.0 added `cell-metrics` + 3 font/cell accessors per
...
tier-hierarchy split. 6.x plugins (6.0.0 / 6.1.0 / 6.2.0) continue to
load against 6.3.0 hosts (additive bumps). Rebuild and reinstall any
...
5. Rebuild and reinstall the `.wasm`. Existing artifacts built against
   `0.25.0` are rejected at load time by `kasane:plugin@6.3.0` hosts.
