...
`kasane:plugin@6.2.0` (adds `cell-metrics` + 3 font/cell accessors per
[#109](https://github.com/Yus314/kasane/issues/109)). ABI 6.1.0 added
`get-display-cells` per [#108](https://github.com/Yus314/kasane/issues/108).
...
tier-hierarchy split. 6.x plugins (6.0.0 / 6.1.0) continue to load
against 6.2.0 hosts (additive bumps). Rebuild and reinstall any plugin
built against 5.x or earlier; those binaries are rejected at load
time. See [`docs/migration/0.6-to-0.7.md` §8.3](migration/0.6-to-0.7.md)
...
5. Rebuild and reinstall the `.wasm`. Existing artifacts built against
   `0.25.0` are rejected at load time by `kasane:plugin@6.2.0` hosts.
