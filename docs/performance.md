...
**Last verified**: 2026-03-23 (tables auto-generated by `tools/update-measurements.sh`)
...
These SLOs function as engineering ratchets ([ADR-024](./decisions.md#adr-024-perception-oriented-performance-policy), Layer 2), not perceptual thresholds. The 200 μs full-frame budget is derived from: the current baseline of ~59 μs leaves 3.4× headroom, and regression up to 200 μs still keeps Kasane well under 1 ms — comfortably below the 4.17 ms display scanout at 240 Hz. Plugin budgets (< 3 μs/plugin) ensure ecosystem scalability: 50 plugins at budget would consume ~150 μs, still under 1 ms.
...
| `element_construct/plugins_10` | view() tree construction (10 plugins) | < 15 us | 9.2 us | OK (1.6× headroom) |
| `flex_layout` | place() layout calculation | < 5 us | 1.2 us | OK (4.1× headroom) |
| `paint/80x24` | clear() + paint() combined | -- | 32.5 us |  paint-only: 27.9 us |
| `paint/200x60` | clear() + paint() combined (large) | -- | 116.3 us |  paint-only: 87.1 us |
| `paint/80x24_realistic` | clear() + paint() combined (varied Face/lengths) | -- | 37.3 us |  paint-only: 32.7 us |
| `grid_clear/80x24` | clear() standalone | -- | 4.6 us |  |
| `grid_clear/200x60` | clear() standalone (large) | -- | 29.2 us |  |
| `grid_diff/full_redraw` | diff() first frame | -- | 30.5 us |  First frame only |
| `grid_diff/incremental` | diff() no changes | -- | 13.5 us |  Steady-state path |
<!-- /BENCH:micro -->
...
| Benchmark | What It Measures | Target | Measured | Verdict |
|---|---|---|---|---|
| `full_frame` | view -> layout -> paint -> diff -> swap | < 16 ms | 59.2 us | OK (270.2× headroom) |
| `draw_message` | state.apply(Draw) + full frame | < 5 ms | 61.9 us | OK (80.7× headroom) |
| `menu_show/items/10` | Menu display + full frame | < 5 ms | 66.8 us | OK (74.8× headroom) |
| `menu_show/items/50` | Menu 50 items + full frame | < 5 ms | 67.5 us | OK (74× headroom) |
| `menu_show/items/100` | Menu 100 items + full frame | < 5 ms | 67.8 us | OK (73.7× headroom) |
| `incremental_edit/lines/1` | 1 line edit -> view + paint + diff | -- | 53.0 us |  |
| `incremental_edit/lines/5` | 5 line edit -> view + paint + diff | -- | 53.9 us |  |
| `message_sequence` | draw_status + draw -> full frame | -- | 62.0 us |  |
<!-- /BENCH:integration -->
...
|---|---|---|---|
| `parse_request/draw_lines/10` | JSON-RPC parse (10-line draw) | 58.4 us |  |
| `parse_request/draw_lines/100` | JSON-RPC parse (100-line draw) | 505.8 us |  |
| `parse_request/draw_lines/500` | JSON-RPC parse (500-line draw) | 2.50 ms |  |
| `parse_request/draw_status` | JSON-RPC parse (draw_status) | 2.7 us | Small message, high frequency |
| `parse_request/menu_show_50` | JSON-RPC parse (menu_show 50 items) | 52.3 us |  |
<!-- /BENCH:parse_request -->
...
| Benchmark | What It Measures | Measured | Notes |
|---|---|---|---|
| `state_apply/draw_lines/23` | state.apply(Draw) standalone | 31.9 us | Includes detect_cursors + compute_lines_dirty |
| `state_apply/draw_lines/100` | state.apply(Draw) standalone | 133.7 us | O(lines × atoms) cursor scanning |
| `state_apply/draw_lines/500` | state.apply(Draw) standalone | 707.9 us |  |
...
| Benchmark | What It Measures | Measured | Notes |
|---|---|---|---|
| `scaling/full_frame/80x24` | Full frame at 80x24 | 57.6 us | Baseline |
| `scaling/full_frame/200x60` | Full frame at 200x60 | 244.5 us |  |
| `scaling/full_frame/300x80` | Full frame at 300x80 | 412.8 us |  |
| `scaling/parse_apply_draw/500` | Parse + apply (500 lines) | 3.21 ms |  |
| `scaling/parse_apply_draw/1000` | Parse + apply (1000 lines) | 6.71 ms | Near-linear |
| `scaling/diff_incremental/80x24` | diff() no-change 80x24 | 12.8 us |  |
| `scaling/diff_incremental/200x60` | diff() no-change 200x60 | 81.5 us |  |
| `scaling/diff_incremental/300x80` | diff() no-change 300x80 | 161.8 us |  |
<!-- /BENCH:scaling -->
...
| Benchmark | What It Measures | Measured | Notes |
|---|---|---|---|
| `e2e_pipeline/json_to_escape_80x24` | Full pipeline (uniform data) | 163.3 us |  |
| `e2e_pipeline/json_to_escape_realistic` | Full pipeline (realistic data) | 118.4 us |  |
<!-- /BENCH:e2e_pipeline -->
...
|---|---|---|---|
| `normal_editing_50msg` | 50 | 4.49 ms | 89.8 us |
| `fast_scroll_100msg` | 100 | 16.58 ms | 165.8 us |
| `menu_completion_20msg` | 20 | 1.69 ms | 84.6 us |
| `mixed_session_200msg` | 200 | 19.32 ms | 96.6 us |
<!-- /BENCH:replay -->
...
|---|---|---|
| `salsa_sync_inputs/buffer_content/23_lines` | 3.1 us | Typical viewport |
| `salsa_sync_inputs/buffer_content/59_lines` | 6.1 us | Large viewport |
| `salsa_sync_inputs/buffer_content/79_lines` | 7.6 us |  |
| `salsa_sync_inputs/buffer_content_realistic_23` | 2.6 us | Realistic face distribution |
| `salsa_sync_inputs/buffer_cursor_only` | 3283.9 ns | Cursor move, no buffer change |
| `salsa_sync_inputs/status` | 3221.6 ns | Status bar update only |
| `salsa_sync_inputs/menu_100_items` | 6.9 us |  |
| `salsa_sync_inputs/all_flags_80x24` | 3.3 us | All flags dirty at 80x24 |
| `salsa_sync_inputs/all_flags_300x80` | 7.8 us | All flags dirty at 300x80 |
<!-- /BENCH:salsa_sync -->
...
|---|---|---|---|
| Full cold (80x24) | 46.4 us | **42.1 us** | +10.3% |
| Menu select warm | **44.6 us** | 51.6 us | -13.7% |
| Incremental edit | 48.4 us | **42.4 us** | +14.1% |
<!-- /BENCH:salsa_vs_legacy -->
...
|---|---|---|---|
| 80×24 | **49.5 us** | 57.6 us | -14.1% |
| 200×60 | **152.9 us** | 244.5 us | -37.5% |
| 300×80 | **250.2 us** | 412.8 us | -39.4% |
<!-- /BENCH:salsa_scaling -->
...
| Benchmark | Measured | Notes |
|---|---|---|
| `line_dirty_single_edit` | 11.5 us | 1 line changed: selective_clear + paint |
| `line_dirty_all_changed` | 9.1 us | All lines dirty: full repaint |
| `line_dirty_buffer_status/1_line_changed` | 11.4 us | BUFFER|STATUS combo, 1 line |
<!-- /BENCH:line_dirty -->
...
|---|---|---|
| `state_apply/draw_lines/23` | 31.9 us | Typical viewport |
| `state_apply/draw_lines/100` | 133.7 us | Large viewport |
| `state_apply/draw_lines/500` | 707.9 us | Exceeds 200 μs budget |
<!-- /BENCH:bottleneck_detect_cursors -->
