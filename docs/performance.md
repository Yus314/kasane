...
**Last verified**: 2026-04-29 (tables auto-generated by `tools/update-measurements.sh`; `parley_pipeline` re-measured manually post-B-wide — see §Parley-only baseline below)
...
> **Parley-only baseline (2026-04-29, post-B-wide)** — captured via
> `cargo bench --bench parley_pipeline`:
>
> | Bench | Time | Target | Δ vs 2026-04-26 |
> |---|---|---|---|
...
> mutex on `StyleStore` plus the `Atom::face()` round-trip on hot
> paths. The B-wide commit (`98592a47`, 2026-04-29) eliminated both
> mechanisms — `Atom` now carries `Arc<UnresolvedStyle>` directly and
> hot-path callers are lock-free pointer dereferences. The
> `frame_one_line_changed_24_lines` measurement after that commit is
> 84.48 µs, statistically indistinguishable from the 83.27 µs
> pre-B-wide number. The gap is real but its cause is elsewhere; the
> next investigation should profile the typing pattern with `dhat-rs`
> or `cargo-instruments` rather than continue fixing the originally
> hypothesised target.
>
> Candidate hot paths to profile next (in priority order): per-frame
> `StyledLine` construction (text alloc + runs / atom_styles vecs),
> `LayoutCache` key hashing on a re-shaped line, and per-glyph
> `GlyphRasterCache` dispatch. None of these were touched by B-wide.
>
> Core rendering benches (`cargo bench --bench rendering_pipeline`)
