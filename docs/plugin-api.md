...
外部プロセス実行、ファイルシステムアクセス、非同期タスク基盤は [Phase P](./roadmap.md#35-phase-p---プラグイン-io-基盤) として計画されている。
...
WASM プラグインは完全にサンドボックス化されている。ホストは `WasiCtxBuilder` にケイパビリティを付与せずに WASM インスタンスを構築するため、ファイルシステム、ネットワーク、プロセス起動などのホスト資源へのアクセスは利用できない。WASM プラグインが利用可能なホスト機能は `host-state`（状態読み取り）と `element-builder`（要素構築）の 2 つの WIT interface に限定される。
