...
外部プロセス実行、ファイルシステムアクセス、非同期タスク基盤は [Phase P](./roadmap.md#35-phase-p---プラグイン-io-基盤) として計画されている。設計根拠は [ADR-019](./decisions.md#adr-019-プラグイン-io-基盤--ハイブリッドモデル) を参照。
...
WASM プラグインはデフォルトでサンドボックス化されている。ホストは `WasiCtxBuilder` にケイパビリティを付与せずに WASM インスタンスを構築するため、ファイルシステム、ネットワーク、プロセス起動などのホスト資源へのアクセスは利用できない。WASM プラグインが利用可能なホスト機能は `host-state`（状態読み取り）と `element-builder`（要素構築）の 2 つの WIT interface に限定される。Phase P ([ADR-019](./decisions.md#adr-019-プラグイン-io-基盤--ハイブリッドモデル)) では、マニフェストで宣言されたケイパビリティに応じて `preopened_dir` / `env` 等を段階的に解放し、プロセス実行はホスト媒介 (`Command::SpawnProcess` + `IoEvent`) で提供する計画である。
...
`PANE_LIFECYCLE`、`PANE_RENDERER`、`SURFACE_PROVIDER`、`WORKSPACE_OBSERVER`、`PAINT_HOOK` は現状 native-only だが、長期的には capability parity を目標とする。ただし同じ trait シグネチャをそのまま WIT に載せることは前提にしない。
...
`SURFACE_PROVIDER` capability を持つ native plugin は独自の surface を提供できる。これは現状 native-only だが、長期的には **WASM parity target** として扱う。`Box<dyn Surface>` をそのまま WIT へ公開するのではなく、hosted surface model として再設計する前提である。
