...
> **注:** 本ドキュメントで特定された課題の多くは、ADR-009 の宣言的 UI アーキテクチャ (Slot / Decorator / Replacement) を通じて、プラグインまたはコア実装で解決される。[宣言的 UI アーキテクチャ設計書](./declarative-ui.md) を参照。
...
Kasane の実装と並行して、上流の Kakoune に提案すべきプロトコル改善。
