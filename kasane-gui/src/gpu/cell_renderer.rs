...
impl CellRenderer {
    ...
    pub fn render(
        ...
        for row in 0..rows {
            let row_hash = compute_row_hash(grid, row as u16, color_resolver);
            ...
            // Build text and span data for this row
            build_row_spans(
                grid,
                row as u16,
                color_resolver,
                &mut self.row_text,
                &mut self.span_ranges,
            );
...
// ---------------------------------------------------------------------------
// Free functions extracted from CellRenderer for benchmarking
// ---------------------------------------------------------------------------
